...
**Demo seeding**: `python manage.py seed` populates a database with the whole demo surface: every role (including a blocked, a soft-deleted, and an unactivated account), a catalog covering every `Course` status (draft/review/needs_revision/rejected/published/hidden/archived plus the `pending_edit` shadow drafts), delivery formats and pricing (USD/EUR/UAH, installments), cohorts and schedules, full curricula with per-course readings and quizzes, enrollments with progress and graded attempts, completions with rendered certificates, reviews, the moderation surface (course approval and rejection records, submitted pending edits, user reports with their immutable action chains, teacher applications, reported reviews, chat rooms with reports and moderation actions), homework with graded submissions, orders/payments/refunds/teacher payouts, and notifications. Content constants live in `apps/common/management/commands/_seed_data/` (Django skips `_`-prefixed packages when discovering commands); `seed.py` itself only decides write order and idempotency.

It is idempotent (`get_or_create` on a natural key, or a seed marker in an existing JSON column for models that have none: `Notification.payload__seed_key`, `UserReport.profile_snapshot__seed_key`, `Order`/`Payment`/`Refund.metadata__seed_key`) and lets the denormalization signals fill `lessons_count` / `students_count` / `rating_*` / `lessons_completed_count`. It never deletes anything, and runs one transaction per section rather than one around the whole command, so it is safe against a shared remote database. `--refresh` additionally rewrites text on rows an earlier run created, guarded by `DEMO_EMAILS` / `DEMO_COURSE_SLUGS` so rows belonging to anyone else are never touched. Moderation history is produced by calling the real services (`CourseService.approve_course` / `reject_course`, `PendingEditService.get_or_create`, `ChatService`) rather than writing records by hand; the services that fan out notifications or email (`UserReportService`, `TeacherApplicationService.approve`) are deliberately bypassed with direct ORM writes. Backdating matters: the moderator dashboard measures review duration from `Course.created_at`, so a course created moments before its approval record reports 0 hours.

**Demo media**: `python manage.py seed_media` is a separate command because it needs the network, must not run in a transaction (rolled-back bytes stay in storage), and writes to whichever storage the *running* environment points at, so the same database needs it run once per environment (local `media/` under `DEBUG=True`, the S3 bucket under `DEBUG=False`). `--download` fetches the committed DiceBear avatars into `apps/common/assets/seed/` (CC0, recorded in that directory's `LICENSE.md`); the default run attaches avatars, generates course covers and teacher signatures with Pillow and lesson document PDFs with reportlab, skipping fields that already have a file behind them unless `--force`. It checks `storage.exists()` rather than the truthiness of the FieldFile, since a column can name a file this environment cannot see.

Neither command is wired into `post_migrate` (that would inject demo rows into prod and the test DB and slow the suite); run them manually after a schema reset: `python manage.py migrate && python manage.py seed && python manage.py seed_media`. When you add or rename a model field the seeder should populate, update `seed.py` (and `_seed_data/`) in the same change.
...
**Configuration**: Settings read from `.env` via `python-decouple`. See `.env.example` for required variables (DB credentials, `SECRET_KEY`, `FRONTEND_URL`, email SMTP settings). `DB_SSLMODE` (default `prefer`) is passed straight to psycopg2: set it to `require` when pointing at a managed instance over the internet, since `prefer` silently falls back to plaintext. To run a command against a remote database, override the vars for that one process (`DB_HOST=... python manage.py ...` or a sourced `.env.rds` in a subshell) rather than editing `.env`, which `runserver` and the devcontainer also read.
