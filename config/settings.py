...
SECURE_BROWSER_XSS_FILTER = (
    True  # Включаем защиту от XSS атак (не даём вписывать скрипты в поля форм и т.д.)
)
X_FRAME_OPTIONS = "DENY"  # Запрещаем отображение сайта в iframe (защита от кликджекинга)
SECURE_CONTENT_TYPE_NOSNIFF = (
    True  # Защита от MIME-атаки (не даём отправлять .exe файлы вместо изображений и т.д.)
)
