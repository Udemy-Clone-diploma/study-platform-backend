    ...
    },
    {
        ...
        "questions": [
            ...
            },
            {
                ...
                "sample_answer": "404",
                "accepted_answers": ["404 not found"],
            },
        ],
    },
    {
        ...
        "questions": [
            ...
            },
            {
                ...
                "sample_answer": "git init",
                "accepted_answers": ["init"],
            },
        ],
    },
]
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        moderators = [
            self._moderator(
                self._user(
                    f"moderator{i}@example.com", User.RoleChoices.MODERATOR, "Mod", f"Erator{i}"
                ),
        ...
        teacher_specs = [
            (
                1,
                "Backend Engineering",
                "10+ years building Django and FastAPI services.",
                Decimal("4.80"),
            ),
            (
                2,
                "Frontend Development",
                "Ex-agency lead focused on React and design systems.",
                Decimal("4.60"),
            ),
            (
                3,
                "Product Design",
                "Mentored 200+ designers into their first role.",
                Decimal("4.90"),
            ),
        ]
        teachers = [
        ...
        student_profiles = [
            ...
            (
                "Top up practical skills alongside a CS degree.",
        ...
        students = [
            self._student(
                self._user(f"student{i}@example.com", User.RoleChoices.STUDENT, "Stu", f"Dent{i}"),
                goals=goals,
                education=edu,
        ...
        e_s0_django = self._enroll(
            students[0], django_course, completed=4, delivery_format=grp_django, granted_days=120
        )
        e_s1_django = self._enroll(
            students[1], django_course, completed=2, delivery_format=grp_django, granted_days=90
        )
        e_s2_django = self._enroll(
            students[2], django_course, completed=0, delivery_format=ind_django, granted_days=20
        )
        self._cohort_member(django_cohort, e_s0_django)
        self._cohort_member(django_cohort, e_s1_django)
        self._schedule_slot(ind_django, 4, time(16, 0), time(17, 0), booked_by=e_s2_django)
        ...
        self._note(
            students[0].user,
            dj_lessons[1],
            "The decorator example finally clicked. Revisit before the quiz.",
            days_ago=70,
        )
        self._note(
            students[1].user, dj_lessons[0], "Reminder: migrations are not optional.", days_ago=50
        )
        self._review(
            django_course, students[0].user, 5, "Best Django course I have taken.", days_ago=80
        )
        self._review(
            django_course, students[1].user, 4, "Very practical, dense in a good way.", days_ago=40
        )
        self._review(
            django_course, students[2].user, 5, "The 1-on-1 sessions were worth it.", days_ago=10
        )
        ...
        react_course = self._course(
            ...
            bullets=[
                "Think in components",
                "Master hooks and state",
                "Fetch data and handle effects",
                "Ship a real single-page app",
            ],
            teacher=teachers[1],
            category=cats["IT"],
        ...
        self._enroll(
            students[1], react_course, completed=100, delivery_format=sp_react, granted_days=120
        )
        self._enroll(
            students[3], react_course, completed=1, delivery_format=sp_react, granted_days=15
        )
        ...
        ux_cohort = self._cohort(
            ...
            group_size=12,
            days_ahead=21,
            delivery_format=grp_ux,
        )
        self._cohort_schedule(ux_cohort, 0, time(19, 0), time(20, 30))
        self._cohort_schedule(ux_cohort, 2, time(19, 0), time(20, 30))
        e_s4_ux = self._enroll(
            students[4], ux_course, completed=1, delivery_format=grp_ux, granted_days=25
        )
        ...
        data_cohort = self._cohort(
            ...
            group_size=20,
            days_ahead=60,
            delivery_format=grp_data,
        )
        self._cohort_schedule(data_cohort, 5, time(11, 0), time(13, 0))
        e_s2_data = self._enroll(
            students[2], data_course, completed=0, delivery_format=grp_data, granted_days=8
        )
        e_s3_data = self._enroll(
            students[3], data_course, completed=2, delivery_format=grp_data, granted_days=35
        )
        ...
        fullstack_course = self._course(
            ...
            bullets=[
                ...
                "Wire up authentication end to end",
            ],
            teacher=teachers[1],
            category=cats["IT"],
            tags=[tags["react"], tags["beginner-friendly"]],
        ...
        self._enroll(
            ...
            fullstack_course,
            completed=2,
            delivery_format=sp_fullstack,
            granted_days=10,
        )
        self._review(
            fullstack_course, students[5].user, 5, "Exactly the project I needed.", days_ago=5
        )
    ...
    def _student(self, user, *, goals="", education="", dob=None):
        return StudentProfile.objects.get_or_create(
            user=user,
            defaults={"learning_goals": goals, "education_level": education, "date_of_birth": dob},
        )[0]
    ...
    def _teacher(self, user, *, specialization="", experience="", rating=Decimal("0.00")):
        return TeacherProfile.objects.get_or_create(
            ...
            defaults={
                ...
                "bio": "Practitioner and mentor.",
                "rating": rating,
            },
        )[0]

    def _moderator(self, user, level="junior"):
        return ModeratorProfile.objects.get_or_create(user=user, defaults={"level": level})[0]
    ...
    def _course(
        ...
        *,
        ...
        status,
        duration_hours,
        language=Course.LanguageChoices.UKRAINIAN,
    ...
    def _curriculum(self, course, *, modules=3, with_meeting=False, with_tests=True):
        ...
        for m in range(1, modules + 1):
            ...
            module, _ = Module.objects.get_or_create(
                course=course,
                order=m,
                defaults={"title": f"Module {m}: {theme}", "description": LOREM[m % len(LOREM)]},
            )

            intro = self._lesson(
                module, 1, f"Introduction to {theme}", preview=(m == 1), meeting=with_meeting
            )
            ...
            if m == 1:
                workshop = self._lesson(
                    module, 4, f"{theme}: hands-on workshop", meeting=with_meeting
                )
    ...
    def _cohort(
        self, course, *, name, months, hours, group_size=None, days_ahead=30, delivery_format=None
    ):
    ...
    def _completion(
        ...
        self._backdate(
            completion, "completed_at", timezone.now() - timedelta(days=completed_days_ago)
        )
    ...
    def _emit_notifications(self, user, templates):
        for t in templates:
            self._notification(
                ...
                t["body"],
                age=AGES[t["age"]],
                seed_key=f"{t['type']}:{t['age']}",
                link_url=t.get("link"),
                actor=t.get("actor"),
                is_read=t.get("read", False),
