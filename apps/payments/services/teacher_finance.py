...
class TeacherFinanceService(PaymentBaseService):
    ...
    def ensure_payment_earning(
        ...
        cls._lock_teacher(payment.teacher_id)

        teacher_amount = cls._decimal_money(Decimal(str(payment.teacher_amount)))

        if teacher_amount <= Decimal("0.00"):
            return None

        entry, _ = TeacherLedgerEntry.objects.get_or_create(
            ...
            defaults={
                ...
                "payment": payment,
                "description": (f"Earning from payment #{payment.id}"),
            },
        )

        # Old/migrated row could theoretically exist
        # as PENDING. Make the operation self-healing.
        if entry.status == TeacherLedgerEntry.StatusChoices.PENDING:
            entry.post()
    ...
    def _lock_teacher(
        ...
        return TeacherProfile.objects.select_for_update().get(pk=teacher_id)
    ...
    def ensure_refund_reservation(
        ...
        payment_amount = cls._decimal_money(Decimal(str(payment.amount)))

        teacher_amount = cls._decimal_money(Decimal(str(payment.teacher_amount)))

        if payment_amount <= Decimal("0.00") or teacher_amount <= Decimal("0.00"):
            return None

        source_key = f"refund:{refund.id}:teacher"

        existing = TeacherLedgerEntry.objects.filter(source_key=source_key).first()
        ...
        # for refund.
        refund_total = payment.refunds.filter(
            status__in=[
        ...
        # active refunds.
        target_teacher_refund = teacher_amount * refund_total / payment_amount

        target_teacher_refund = cls._decimal_money(target_teacher_refund)
        ...
        existing_adjustments = TeacherLedgerEntry.objects.filter(
            payment=payment,
        ...
        already_reserved = abs(cls._decimal_money(existing_adjustments))

        adjustment = cls._decimal_money(target_teacher_refund - already_reserved)
        ...
        entry = TeacherLedgerEntry.objects.create(
            ...
            description=(f"Refund adjustment for payment #{payment.id}"),
            metadata={
                "payment_amount": (f"{payment_amount:.2f}"),
                "teacher_amount": (f"{teacher_amount:.2f}"),
    ...
    def post_refund_adjustment(
        ...
        entry = cls.ensure_refund_reservation(refund)

        if entry is None:
            return None

        if entry.status == TeacherLedgerEntry.StatusChoices.VOID:
    ...
    def void_refund_adjustment(
        ...
        entry = (
            TeacherLedgerEntry.objects.select_for_update()
            .filter(source_key=(f"refund:{refund.id}:teacher"))
            .first()
        )
        ...
        if entry.status == TeacherLedgerEntry.StatusChoices.POSTED:
    ...
    def balance(
        ...
        *,
        ...
        posted = entries.filter(status=TeacherLedgerEntry.StatusChoices.POSTED)

        pending = entries.filter(status=TeacherLedgerEntry.StatusChoices.PENDING)

        earned = cls._sum_entries(
            posted.filter(entry_type=(TeacherLedgerEntry.TypeChoices.EARNING))
        )

        refund_total = cls._sum_entries(
            posted.filter(entry_type=(TeacherLedgerEntry.TypeChoices.REFUND))
        )

        payout_total = cls._sum_entries(
            posted.filter(entry_type=(TeacherLedgerEntry.TypeChoices.PAYOUT))
        )

        adjustment_total = cls._sum_entries(
            posted.filter(entry_type=(TeacherLedgerEntry.TypeChoices.ADJUSTMENT))
        )

        posted_balance = cls._sum_entries(posted)
        ...
        pending_negative = cls._sum_entries(pending.filter(amount__lt=Decimal("0.00")))

        reserved = abs(pending_negative)

        available = cls._decimal_money(posted_balance - reserved)

        return {
            "currency": currency,
            "earned": cls._decimal_money(earned),
            "refunded": cls._decimal_money(abs(refund_total)),
            "paid": cls._decimal_money(abs(payout_total)),
            "adjustments": cls._decimal_money(adjustment_total),
            "reserved": cls._decimal_money(reserved),
            "balance": cls._decimal_money(posted_balance),
            "available": available,
        }
    ...
    def reserve_payout(
        ...
        *,
        ...
        if not currency:
            raise PaymentError("Payout currency is required.")

        if not idempotency_key:
            raise PaymentError("Payout idempotency key is required.")

        teacher = cls._lock_teacher(teacher.id)

        # Idempotent retry must return the original payout
        # when all immutable request parameters match.
        existing = TeacherPayout.objects.filter(idempotency_key=idempotency_key).first()
        ...
        if existing is not None:
            if (
                ...
                raise PaymentError("Payout idempotency key is already used for another payout.")

            return existing

        if destination.teacher_id != teacher.id:
            raise PaymentError("Payout destination does not belong to this teacher.")

        if not destination.is_active:
            raise PaymentError("Payout destination is inactive.")

        if destination.provider != provider:
            raise PaymentError("Payout destination provider does not match payout provider.")

        cls._validate_payout_destination(destination)
        ...
        if destination.destination_type == TeacherPayoutDestination.TypeChoices.BANK_ACCOUNT:
            destination_snapshot.update(
                {
                    "receiver_account": (destination.receiver_account),
                    "receiver_mfo": (destination.receiver_mfo),
                    "receiver_okpo": (destination.receiver_okpo),
                    "receiver_company": (destination.receiver_company),
                }
            )

        elif destination.destination_type == TeacherPayoutDestination.TypeChoices.CARD_TOKEN:
            destination_snapshot.update(
                {
                    "receiver_card_token": (destination.receiver_card_token),
                }
            )
        ...
        for earning in earning_entries:
            ...
            refund_adjustments = TeacherLedgerEntry.objects.filter(
                payment=payment,
                entry_type=(TeacherLedgerEntry.TypeChoices.REFUND),
                status__in=[
                    TeacherLedgerEntry.StatusChoices.PENDING,
            ...
            already_allocated = TeacherPayoutItem.objects.filter(
                payment=payment,
                payout__status__in=(active_payout_statuses),
            ).aggregate(total=Sum("amount")).get("total") or Decimal("0.00")

            source_available = cls._decimal_money(
                earning.amount + refund_adjustments - already_allocated
            )
            ...
            allocation = cls._decimal_money(allocation)
            ...
            remaining = cls._decimal_money(remaining - allocation)

        if remaining != Decimal("0.00"):
            raise PaymentError(
                "Available teacher balance could not be allocated to source payments."
            )
        ...
        TeacherLedgerEntry.objects.create(
            ...
            description=(f"Teacher payout #{payout.id}"),
        )
    ...
    def mark_payout_processing(
        ...
        *,
        ...
        cls._lock_teacher(payout.teacher_id)
        ...
        if payout.status in {
            ...
            raise PaymentError("Finished payout cannot be moved to processing.")
        ...
        if ledger is None:
            raise PaymentError("Payout ledger reservation was not found.")

        if ledger.status != TeacherLedgerEntry.StatusChoices.PENDING:
            raise PaymentError("Payout reservation is not pending.")
    ...
    def _validate_payout_destination(
        ...
        if destination.destination_type == TeacherPayoutDestination.TypeChoices.BANK_ACCOUNT:
            ...
            if missing:
                raise PaymentError("Bank payout destination is incomplete.")

            return

        if destination.destination_type == TeacherPayoutDestination.TypeChoices.CARD_TOKEN:
            if not destination.receiver_card_token:
                raise PaymentError("LiqPay card token is required.")

            return

        raise PaymentError("Unsupported payout destination type.")
    ...
    def mark_payout_succeeded(
        ...
        *,
        ...
        if payout.status in {
            ...
            raise PaymentError("Failed or canceled payout cannot be marked as succeeded.")
        ...
        if ledger.status == TeacherLedgerEntry.StatusChoices.VOID:
            raise PaymentError("Voided payout reservation cannot be settled.")
    ...
    def mark_payout_failed(
        ...
        *,
        ...
        if payout.status == TeacherPayout.StatusChoices.SUCCEEDED:
            raise PaymentError("Successful payout cannot be marked as failed.")
        ...
        if ledger.status == TeacherLedgerEntry.StatusChoices.POSTED:
            raise PaymentError("Settled payout ledger entry cannot be voided.")
