...
class PayoutExecutionService:
    ...
    def _provider_for_mode(
        ...
        raise PaymentError(
            "Unsupported teacher payout mode."
        )
    ...
    def _prepare_payout_execution(
        ...
        *,
        ...
        TeacherFinanceService._lock_teacher(
            payout.teacher_id
        )
        ...
        if payout.status in {
            ...
            raise PaymentError(
                "Finished payout cannot be executed."
            )
        ...
        if (
            ...
            if not existing_mode:
                raise PaymentError(
                    "Processing payout has no recorded "
                    "execution mode."
                )
    ...
    def execute_teacher_payout(
        ...
        *,
        ...
        provider = cls._provider_for_mode(
            payout_mode
        )

        if (
            payout_mode == "liqpay_sandbox"
            and not str(client_ip or "").strip()
        ):
            raise PaymentError(
        ...
        if (
            result.status
            == LiqPaySandboxPayoutProvider.SUCCEEDED
        ):
            return (
                TeacherFinanceService
        ...
        if (
            result.status
            == LiqPaySandboxPayoutProvider.FAILED
        ):
            reason = str(
                result.raw.get(
                    "err_description"
                )
                or "Provider rejected payout."
            )
    ...
    def handle_liqpay_payout_callback(
        ...
        *,
        ...
        if not LiqPayService._liqpay_verify_signature(
            ...
            raise PaymentError(
                "Invalid LiqPay payout "
                "callback signature."
            )

        payload = (
            LiqPayService._liqpay_decode_data(
                data
            )
        )
        ...
        if payout_mode != "liqpay_sandbox":
            raise PaymentError(
                "Teacher payout was not "
                "executed through LiqPay sandbox."
            )
