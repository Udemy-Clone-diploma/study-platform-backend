...
class CohortAttendanceView(GenericAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if not date_str:
            return Response(
                {"detail": "date query param required."}, status=status.HTTP_400_BAD_REQUEST
            )
        try:
        ...
        month_session_ids = list(
            Session.objects.filter(
                cohort_q, status=ACTIVE, date__year=now.year, date__month=now.month
            ).values_list("id", flat=True)
        )
        all_session_ids = list(
            Session.objects.filter(cohort_q, status=ACTIVE, date__lte=today).values_list(
                "id", flat=True
            )
        )
        ...
        def _present_by(session_ids: list) -> dict[int, int]:
            ...
            for row in (
                Attendance.objects.filter(
                    session_id__in=session_ids, enrollment_id__in=enrollment_ids, is_present=True
                )
            ...
            result.append(
                {
                    ...
                    "progress_percent": round(enr.lessons_completed_count / total_lessons * 100)
                    if total_lessons
                    else 0,
                    "monthly_attendance_percent": round(
                        present_month.get(enr.id, 0) / total_month * 100
                    )
                    if total_month
                    else 0,
                    "total_attendance_percent": round(present_all.get(enr.id, 0) / total_all * 100)
                    if total_all
                    else 0,
                }
            )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not date_str or enrollment_id is None:
            return Response(
                {"detail": "date and enrollment_id are required."},
        ...
        enrollment = get_object_or_404(
            Enrollment, id=enrollment_id, cohort_memberships__cohort=cohort
        )
...
class IndividualEnrollmentListView(GenericAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        enrollments = Enrollment.objects.filter(
            course=course, delivery_format__format_type="individual"
...
class IndividualSessionDatesView(GenericAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        dates = (
            Session.objects.filter(
                slot__booked_by=enrollment, status=ACTIVE, date__year=year, date__month=month
            )
            .values_list("date", flat=True)
            .distinct()
            .order_by("date")
...
class IndividualAttendanceView(GenericAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        present_month = (
            Attendance.objects.filter(
                session_id__in=month_ids, enrollment=enrollment, is_present=True
            ).count()
            if total_month
            else 0
        )
        present_all = (
            Attendance.objects.filter(
                session_id__in=all_ids, enrollment=enrollment, is_present=True
            ).count()
            if total_all
            else 0
        )
            ...
            [
                {
                    ...
                    "progress_percent": round(
                        enrollment.lessons_completed_count / total_lessons * 100
                    )
                    if total_lessons
                    else 0,
                    "monthly_attendance_percent": round(present_month / total_month * 100)
                    if total_month
                    else 0,
                    "total_attendance_percent": round(present_all / total_all * 100)
                    if total_all
                    else 0,
                }
            ]
    ...
    def post(self, request, *args, **kwargs):
        ...
        session = get_object_or_404(
            Session, slot__booked_by=enrollment, date=session_date, status=ACTIVE
        )
        attendance, _ = Attendance.objects.update_or_create(
            session=session,
            enrollment=enrollment,
