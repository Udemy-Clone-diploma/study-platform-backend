...
class AuthRegisterEndpointTests(APITestCase):
    ...
    def test_register_response_omits_password(self):
        with patch("apps.users.services.email_service.EmailService.send_verification_email"):
            response = self.client.post(self.url, self.payload)
    ...
    def test_register_force_role_to_student(self):
        ...
        with patch("apps.users.services.email_service.EmailService.send_verification_email"):
            response = self.client.post(self.url, {**self.payload, "role": "teacher"})
    ...
    def test_register_rejects_duplicate_email(self):
        User.objects.create_user(email=self.payload["email"], password="x", role="student")
